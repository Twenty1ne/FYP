...
int main(){
    ...
    // burn in period to let the system reach a critical point
...
Avalanche_Stats topple(std::vector<std::vector<int>>& grid){
    ...
    // continues to topple while the grid has sites above 4
    bool unstable = true;
    while(unstable){
        unstable = false;
...
void save_avalanche_stats(const std::vector<Avalanche_Stats>& stats){
    // saving avalanche data to file to plot with python
