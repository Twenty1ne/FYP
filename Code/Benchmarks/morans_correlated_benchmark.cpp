...
void populate_grid(std::vector<std::vector<Site>>& polar_grid){
    ...
    for(int r = 1; r <= NUMBER_OF_RINGS; r++){
        ...
        for(int site = 0; site < numStars; site++){
            ...
            // populates one half of the grid completely
            unsigned int star_size = (site <= (numStars / 2)) ? 0 : 10;
...
void save_galaxy_history(const std::vector<std::vector<Site>>& galaxy_history){
    ...
    std::ofstream outfile("Morans_I_Correlated_Benchmark.txt");
    if(outfile.is_open()){
        ...
        outfile << std::endl;
        outfile.close();
        std::cout << "Morans_I_Correlated_Benchmark.txt" << std::endl;
    }
    else{
