...
improves the definition of the baryon cycle
now allows gas to become a cloud again for a further 5Myr where it can be triggered early by a supernova
otherwise it will collapse on its own without any external perturbations
*/

// constants to be used throughout the simulation
const int NUMBER_OF_RINGS = 50;
const int TIME_STEPS = 1500;
const double PI = 3.14159265358979323846;
const double SPONTANEOUS_FORMATION = 0.001;
const int HEATED_GAS_COOLDOWN = 15;
const int COOLED_GAS_COUNTDOWN = 5;
...
struct Site{
    ...
    unsigned short heated_gas_cooldown;
    bool recently_cooled;
    unsigned short cooled_gas_countdown;

    // member function to alter the various mass components and distribute heated gas to neighbouring sites
    void supernova(std::vector<std::vector<Site>>& polar_grid){
        ...
        for(const auto& [r, angle] : nbrs){
            ...
            polar_grid[r][angle].heated_gas_cooldown = HEATED_GAS_COOLDOWN;
        }
    }
};
...
unsigned int mass_power_law(double beta);
...
// populates the grid with a radial gradient profile matching literature
void populate_grid(std::vector<std::vector<Site>>& polar_grid){
    std::vector<Site> ring;
    ring.push_back({0, 0, 0, 10000000, 0, 0, 0, 0, false, false, 0, 0, 0, false, COOLED_GAS_COUNTDOWN});
    ...
    for(int r = 1; r <= NUMBER_OF_RINGS; r++){
        ...
        for(int site = 0; site < numStars; site++){
            ...
            unsigned int cold_gas = (myRand() >= exp(-200*(1.0986e-4)*(r + 0.5))) ? 0 : mass_power_law(1.7);
            ring.push_back({static_cast<unsigned short>(r), static_cast<unsigned short>(site), theta, cold_gas, 0, 0, 0, 0, false, false, 0, 0, 0, false, COOLED_GAS_COUNTDOWN});
        }
        polar_grid.push_back(ring);
    }
}

// power law distribution of GMC masses matching with literature (expected mass = 163,307 solar masses)
unsigned int mass_power_law(double beta){
    const int xmin = 10000;
    const int xmax = 10000000;
    return static_cast<unsigned int>(pow((pow(xmax, -beta+1) - pow(xmin, -beta+1)) * myRand() + pow(xmin, -beta+1), 1.0 / (-beta+1)));
}
...
// GMCs collapse here and form a cluster of stars following a distribution based on literature
void gmc_collapse(std::vector<std::vector<Site>>& polar_grid){
    for(int r = 0; r < polar_grid.size(); r++){
        for(auto& site : polar_grid[r]){
            if(site.cold_gas >= 10000 && !site.contains_active_star && site.nearby_supernova){
                ...
                site.cold_gas = 0;
                site.heated_gas_cooldown = HEATED_GAS_COOLDOWN;
                site.cooled_gas_countdown = COOLED_GAS_COUNTDOWN;
            }
        }
    }
}

// function to handle differential rotation and data tracking
void rotate(std::vector<std::vector<Site>>& polar_grid, std::vector<std::vector<unsigned long>>& galaxy_mass, std::string& galaxy_string, std::string& correlation_string){
    ...
    if(polar_grid[0][0].heated_gas_cooldown > 0){
        polar_grid[0][0].heated_gas_cooldown--;
    }
    ...
    for(int r = 1; r < polar_grid.size(); r++){
        ...
        for(int i = 0; i < numStars; i++){
            ...
            if(polar_grid[r][i].heated_gas_cooldown > 0){
                polar_grid[r][i].heated_gas_cooldown--;
            }
            if(polar_grid[r][i].cooled_gas_countdown == 0){
                polar_grid[r][i].nearby_supernova = true;
            }
            if(polar_grid[r][i].recently_cooled && polar_grid[r][i].cooled_gas_countdown > 0){
                polar_grid[r][i].cooled_gas_countdown--;
            }

            //allow heated gas to become cold gas after its cooled
            if(polar_grid[r][i].heated_gas_cooldown == 0 && polar_grid[r][i].heated_gas > 0){
                polar_grid[r][i].cold_gas += polar_grid[r][i].heated_gas;
                polar_grid[r][i].heated_gas = 0;
                polar_grid[r][i].recently_cooled = true;
                polar_grid[r][i].cooled_gas_countdown = COOLED_GAS_COUNTDOWN;
            }
...
void save_galaxy_history(const std::vector<std::string>& galaxy_history){
    ...
    std::ofstream outfile("MBSOGM_III_history.txt");
    if (!outfile.is_open()) {
    ...
    outfile.close();
    std::cout << "Data written to MBSOGM_III_history.txt\n";
}

// saves the mass component history for plotting
void save_mass_history(const std::vector<std::vector<unsigned long>>& galaxy_mass){
    std::cout << "Writing Mass History" << std::endl;
    std::ofstream outfile("MBSOGM_III_mass_history.txt");
    if(outfile.is_open()){
        ...
        outfile.close();
        std::cout << "Data written to MBSOGM_III_mass_history.txt" << std::endl;
    }
    else{
...
void save_correlation_history(const std::vector<std::string>& correlation_history){
    ...
    std::ofstream outfile("MBSOGM_III_correlations_history.txt");
    if (!outfile.is_open()) {
        std::cout << "Error opening file\n";
        return;
    }
    ...
    outfile.close();
    std::cout << "Data written to MBSOGM_III_correlations_history.txt\n";
}